#!/usr/bin/env python3
#
# Simple file operations for a FAT16 floppy disk image.
# Public domain License.
# The project was forked from https://github.com/bbbradsmith/makeflop.py and modified for FAT16 file system
...
class Floppy:
    ...
    def _filestring(s, length):
        ...
        if len(b) != length:
            raise Floppy.Error("File string '%s' too long? (%d != %d)" % (s, len(b), length))
        return b
    ...
    class FileEntry:
        ...
        def __init__(self, data=None, dir_cluster=-1, dir_index=-1):
            ...
            self.path = ""
            self.long_path = ""
            self.has_lfn_entry = 0
            self.incipit = data[0]
            self.attributes = data[11]
            if self.attributes != 0x0F:
                self.short_file_entry(data, dir_cluster, dir_index)
            else:
                self.long_file_entry(data)

        def short_file_entry(self, data, dir_cluster=-1, dir_index=-1):
            """Unpacks a 32 byte directory entry into a Short FileEntry structure."""
            if self.incipit != 0x00 and self.incipit != Floppy.EMPTY:
                ...
                self.path = filename if not self.long_path else self.long_path
                if len(extension) > 0:
        ...
        def long_file_entry(self, data):
            """Unpacks a 32 byte directory entry into a LongFileEntry structure."""
            self.has_lfn_entry = 1
            self.lfn_entry_number = -1
            sequenceNo = data[0]
            checksum = data[13]

            if sequenceNo | 0x40 == sequenceNo:
                # this is the last LFN entry
                self.lfn_entry_number = sequenceNo - 0x40
            else:
                # for all other LFN entries
                self.lfn_entry_number = sequenceNo

            filename = bytes(filter(lambda x: x != 0xFFFF, list(struct.unpack('HHHHH', data[1:11])))).decode("ASCII")
            filename = filename.rstrip(" ").rstrip("\0")
            filename += bytes(filter(lambda x: x != 0xFFFF, list(struct.unpack('HHHHHH', data[14:26])))).decode("ASCII")
            filename = filename.rstrip(" ").rstrip("\0")
            filename += bytes(filter(lambda x: x != 0xFFFF, list(struct.unpack('HH', data[28:32])))).decode("ASCII")
            filename = filename.rstrip(" ").rstrip("\0")
            self.long_path = filename
    ...
    def _boot_open(self):
        ...
        if self.data[38] == 0x29 and len(self.data) >= 54:
            ...
            self.file_system_type = self.data[54:61].decode("ASCII").rstrip(" ")
        ...
        root_sectors = (self.root_max * 32) // self.sector_size  # round up to fill sector
    ...
    def _fat_open(self):
        ...
        while (e + 2) <= fat_end:
            ...
            # for FAT16
            entry = (self.data[e + 0]) | (self.data[e + 1] << 8)
            e += 2
    ...
    def _delete_chain(self, cluster):
        """Deletes a FAT chain."""
        while cluster < 0xFFF0 and cluster >= 2:
    ...
    def _add_chain(self, data):
        ...
        self.fat[chain[len(chain) - 1]] = 0xFFFF
    ...
    def _files_dir(self, cluster, path):
        ...
        for i in range(len(directory) // 32):
            ...
            if e.incipit == Floppy.EMPTY:  # empty
                continue
            if e.attributes == 0x0F:  # long file name
                if LFN_entry_remaining:
                    LFN_entry_remaining = e.lfn_entry_number - 1
                    long_path = e.long_path + long_path
                else:
            ...
            if (e.attributes & 0x10) != 0:  # subdirectory
                ...
                if not LFN_entry_remaining:
                    # clear LFN counters when LFN entries are complete for an item
                    LFN_entry_remaining = 1
                    long_path = ""
                else:
                    pass
                    # print('LFN entries remaining for subdirectory')
            else:
                if '~' in (path + e.path):
                    entries.append(path + long_path.strip(" "))
                else:
                ...
                if not LFN_entry_remaining:
                    ...
                    pass
                    # print('LFN entries remaining for file')
        return entries
    ...
    def _dir_entry_offset(self, cluster, dir_index):
        ...
        bytes_per_dir_entry = (self.sector_size * self.cluster_sects) // 32
        if (dir_index < bytes_per_dir_entry):  # within this cluster
            return self._cluster_offset(cluster) + (32 * dir_index)
        # continue to next cluster
        return self._dir_entry_offset(self.fat[cluster], dir_index - bytes_per_dir_entry)
    ...
    def delete_file(self, entry):
        ...
        if entry.has_lfn_entry:
            offset = offset - 32
            while (offset | 0x40) != offset:
                self.data[offset + 0] = Floppy.EMPTY  # empty this LFN entry
                offset = offset - 32

            if (offset | 0x40) == offset:
                self.data[offset + 0] = Floppy.EMPTY  # empty the last LFN entry
    ...
    def _add_entry(self, dir_cluster, entry):
        ...
        if i >= dir_len:
            ...
            tail = dir_cluster
            while self.fat[tail] < 0xFFF0:
                tail = self.fat[tail]
            self.fat[tail] = chain
            self.fat[chain] = 0xFFFF
        # insert entry
    ...
    def _add_dir_recursive(self, cluster, path):
        ...
        for i in range(len(directory) // 32):
            ...
            if e.incipit == Floppy.EMPTY:  # empty
                continue
            if e.attributes == 0x0F:  # long file name entry
                continue
            if (e.attributes & 0x10) != 0:  # subdirectory
    ...
    def set_volume_label(self, label):
        ...
        self.data[54:62] = Floppy._filestring("FAT16", 8)
